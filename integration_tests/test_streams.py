...
from source_waba_analytics.source import SourceWabaAnalytics
...
class TestFacebookMarketingSource:
    ...
    def _read_records(conf, catalog, state=None) -> Tuple[List[AirbyteMessage], List[AirbyteMessage]]:
        records = []
        states = []
        for message in SourceWabaAnalytics().read(logging.getLogger("airbyte"), conf, catalog, state=state):
