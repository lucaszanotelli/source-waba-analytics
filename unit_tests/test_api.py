...
class TestMyFacebookAdsApi:
    @pytest.fixture
    def fb_api(self):
        return source_waba_analytics.api.MyFacebookAdsApi.init(access_token="foo", crash_log=False)
    ...
    def test__handle_call_rate_limit(self, mocker, fb_api, params, min_rate, usage, expect_sleep):
        ...
        mocker.patch.object(source_waba_analytics.api, "logger")
        mocker.patch.object(source_waba_analytics.api, "sleep")
        ...
        if expect_sleep:
            ...
            source_waba_analytics.api.sleep.assert_called_with(fb_api._compute_pause_interval.return_value.total_seconds())
            source_waba_analytics.api.logger.warning.assert_called_with(
