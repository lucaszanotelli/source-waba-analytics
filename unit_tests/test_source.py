...
from source_waba_analytics.spec import ConnectorConfig
...
def api_fixture(mocker):
    api_mock = mocker.patch("source_waba_analytics.source.API")
...
def logger_mock_fixture(mocker):
    return mocker.patch("source_waba_analytics.source.logger")


class TestSourceWabaAnalytics:
    def test_check_connection_ok(self, api, config, logger_mock):
        ok, error_msg = SourceWabaAnalytics().check_connection(logger_mock, config=config)
    ...
    def test_check_connection_end_date_before_start_date(self, api, config, logger_mock):
        ...
        assert SourceWabaAnalytics().check_connection(logger_mock, config=config) == (
            False,
    ...
    def test_check_connection_empty_config(self, api, logger_mock):
        ...
        with pytest.raises(pydantic.ValidationError):
            SourceWabaAnalytics().check_connection(logger_mock, config=config)
    ...
    def test_streams(self, config, api):
        streams = SourceWabaAnalytics().streams(config)

        assert len(streams) == 16

    def test_spec(self):
        spec = SourceWabaAnalytics().spec()
    ...
    def test_get_custom_insights_streams(self, api, config):
        ...
        assert SourceWabaAnalytics().get_custom_insights_streams(api, config)
    ...
    def test_get_custom_insights_action_breakdowns_allow_empty(self, api, config):
        ...
        streams = SourceWabaAnalytics().get_custom_insights_streams(api, ConnectorConfig.parse_obj(config))
