...
class WhatsAppConversations(FBMarketingIncrementalStream):
    """doc: https://developers.facebook.com/docs/graph-api/reference/waba-analytics/"""

    entity_prefix = "whats_app_conversations"
    primary_key = None
    cursor_field = "start"

    def list_objects(self, params: Mapping[str, Any]) -> Iterable:
        params |= {
            'start': floor(datetime.timestamp(self._start_date)),
            'end': floor(datetime.timestamp(self._end_date)),
            'granularity': self.granularity,
            }
        return self._api.account.get_conversation_analytics(params=params)
