...
if TYPE_CHECKING:  # pragma: no cover
    from source_waba_analytics.api import API
...
class FBMarketingStream(Stream, ABC):
    ...
    def read_records(
        ...
        loaded_records_iter = self.list_objects(params=self.request_params(stream_state=stream_state))
        if self.use_batch:
...
class FBMarketingIncrementalStream(FBMarketingStream, ABC):
    ...
    def __init__(self, start_date: datetime, end_date: datetime, **kwargs):
        ...
        if self.entity_prefix == "whats_app_conversations":
